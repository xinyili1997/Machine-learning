...
class help_function:
	...
	# function for 1. part - read infile
	def read_infile(csv_file):
	...
	# function for 2. and 3. parts - read infile and ture the datasets into 2 dictionaries
	def read_dataset(csv_file):
		...
		"""
		datasets, attrs, label = help_function.read_infile(csv_file)
	...
	# function for compare prediction results with true results
	def label_comparison(infile, outfile, Dtree):
		output_file = open(outfile, "w")
		errorNum = 0
		attr_dictionary_list, label_list = help_function.read_dataset(infile)
		for attr_dictionary, label in zip(attr_dictionary_list, label_list):
			prediction = Dtree.predict(attr_dictionary)
			if prediction != label:
				errorNum += 1
			output_file.write("{}\n".format(prediction))
		output_file.close()
		error_rate = errorNum / len(label_list)
		return error_rate
...
class decisionTree_lite:
	...
	def read_train_infile(self, train_infile):
		datasets, attrs, label = help_function.read_infile(train_infile)
		return datasets, attrs, label
...
def main():
	...
	* 4 parts are involved:
	...
	train_error_rate = help_function.label_comparison(train_infile, train_label_outfile, tree)

	# 3. Read test dataset and predict output: test_label_outfile
	test_error_rate = help_function.label_comparison(test_infile, test_label_outfile, tree)
